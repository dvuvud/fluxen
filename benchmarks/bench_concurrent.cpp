...
sqlite3 *open_sqlite_wal(const std::string &path) {
  sqlite3 *db = nullptr;
...
void sqlite_populate(const std::string &path,
                     const std::vector<std::string> &keys,
                     const std::vector<std::string> &vals) {
  sqlite3 *db = open_sqlite_wal(path);
  sqlite3_exec(
      db, "CREATE TABLE IF NOT EXISTS kv (k TEXT PRIMARY KEY, v TEXT); BEGIN;",
      nullptr, nullptr, nullptr);
  sqlite3_stmt *stmt = nullptr;
  sqlite3_prepare_v2(db, "INSERT OR REPLACE INTO kv VALUES (?, ?);", -1, &stmt,
                     nullptr);
...
static void BM_fluxen_ReadWriteContention(benchmark::State &state) {
  ...
  state.counters["reads"] =
      benchmark::Counter(local_reads, benchmark::Counter::kIsRate);
}
...
static void BM_SQLite_ReadWriteContention(benchmark::State &state) {
  ...
  if (state.thread_index() == 0) {
    ...
    sqlite3_prepare_v2(db, "INSERT OR REPLACE INTO kv VALUES (?, ?);", -1, &ins,
                       nullptr);
    for (auto _ : state) {
      for (int i = 0; i < n; ++i) {
