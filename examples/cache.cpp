...
//
// Usage:
//   cache run <command>          run command, print output (cached after first
//   run) cache invalidate <command>   bust the cache for a specific command
...
static std::string run_command(const std::string &cmd) {
  ...
  if (exit_code != 0) {
    throw std::runtime_error("cache: command exited with code " +
...
static void print_usage() {
  std::cout
      << "usage:\n"
         ...
         "  cache invalidate <command>   remove a specific entry from the "
         "cache\n"
...
int main(int argc, char *argv[]) {
  ...
  if (cmd == "list") {
    ...
    db.each([](std::string_view key, fluxen::Bytes) {
      std::cout << key << "\n";
    });
